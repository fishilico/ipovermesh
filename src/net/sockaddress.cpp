...
namespace iom
{
    ...
    :ip(), port(0) {
        ...
        if (addressFormat.at(0) == '[') {
            ...
            ip = Address(addressFormat.substr(1, n - 1), 6);
            portstr = addressFormat.substr(n + 1);
        } else {
            //If not IPv6, it's IPv4, obviously :P
            size_t n = addressFormat.find(':');
            if (n == std::string::npos) {
                ip = Address(addressFormat, 4);
            } else {
                ip = Address(addressFormat.substr(0, n), 4);
    ...
    SockAddress::SockAddress(const Address& addr, unsigned short port)
    :ip(addr), port(port) {
    }
    ...
    const Address& SockAddress::getIp() const {
        return ip;
    }
